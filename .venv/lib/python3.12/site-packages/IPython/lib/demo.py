...
The classes are (see their docstrings for further details):
 ...
 - IPythonDemo: demos with input to be process by IPython as if it had been
 ...
 - IPythonLineDemo: IPython version of the LineDemo class (the demo is
   executed a line at a time, but process via IPython).
...
class IPythonLineDemo(IPythonDemo,LineDemo):
    """Variant of the LineDemo class whose input is process by IPython."""
    pass
