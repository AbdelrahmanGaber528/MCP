...
class PyDB(object):
    ...
    PyDB communicates with these threads through command queues.
       Every RDB command is process by calling process_net_command.
        ...
        with self._main_lock:
            ...
            if len(program_threads_alive) == 0 and ready_to_run:
                ...
                for thread_id in process_thread_ids:
                    ...
                    # some commands must be process by the thread itself... if that's the case,
    ...
    def dispose_and_kill_all_pydevd_threads(self, wait=True, timeout=0.5):
        ...
        try:
            ...
            # Wait until a time when there are no commands being process to kill the threads.
            started_at = time.time()
            while time.time() < started_at + timeout:
                with self._main_lock:
                    writer = self.writer
                    if writer is None or writer.empty():
                        pydev_log.debug("PyDB.dispose_and_kill_all_pydevd_threads no commands being process.")
                        break
            else:
