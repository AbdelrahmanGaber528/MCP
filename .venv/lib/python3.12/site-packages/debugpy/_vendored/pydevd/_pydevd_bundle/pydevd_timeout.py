...
class _TimeoutThread(PyDBDaemonThread):
    ...
    def process_handles(self):
        ...
        with self._lock:
            ...
            # of processing time -- anything not process now may be process at the
            # next snapshot).
            curtime = time.time()
            ...
            for handle in handles:
                if curtime < handle.abs_timeout and not handle.disposed:
                    ...
                    if _DEBUG:
                        pydev_log.critical("pydevd_timeout: Handle NOT process: %s", handle)
                    ...
                    if _DEBUG:
                        pydev_log.critical("pydevd_timeout: Handle process: %s", handle)
